...
di1 = @(t,i1)((((sqrt(2)*110*sin(2*pi*60*t))/(L11+Lm)-(r1*i1)/(L11+Lm)))); % declaring FUNODE to solve differential equation using ODE
[t0,i1] = ode23(di1,[0,0.2],0);
v2=(sqrt(2)*110*sin(2*pi*60*t0))*(Lm/(L11+Lm)) - i1*(r1*((L11+Lm+1)/(L11+Lm)));
...
title('TRANSFORMER MODELING - OPEN CIRCUIT [ v1=sqrt(2)110sin(377t) ]')
xlabel('time (s)')
ylabel('v1 (V)')
grid on
