...
f=polyval(p,x1);
i2=-i1; 
lambdam=vpa(p(1).*(i1+i2).^6+p(2).*(i1+i2).^5+p(3).*(i1+i2).^4+p(4).*(i1+i2)^3+p(5).*(i1+i2)^2+p(6).*(i1+i2)+p(7));% lambdam in plynomial form of six degree 
...
di1=@(t,i1) (((-r1*i1)/(L11+eval(dlambdam)))+((sqrt(2)*110*sin(2*pi*60*t))/(L11+eval(dlambdam))));
[t0,i1] = ode15s(di1,[0.0,0.2],0.0);

i2=-i1;                                                                               % i2=0 since short circuit
v2=0;
...
title('TRANSFORMER MODELING - OPEN CIRCUIT WITH SATURATION [ v1=sqrt(2)110sin(377t) ]')
xlabel('time (s)')
ylabel('v1 (V)')
grid on
